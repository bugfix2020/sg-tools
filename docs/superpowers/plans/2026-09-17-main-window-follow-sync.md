...
- Do not modify `Desktop/clicker` reference sources, commit, or push.
...
- [ ] **Step 1: Write the failing filter and conflict tests**
...
- [ ] **Step 2: Run the new tests and confirm the feature is absent**
...
- [ ] **Step 3: Add the minimal types, errors, normalization, and filter implementation**
...
- [ ] **Step 4: Run the focused tests and the existing clicker tests**
...
- [ ] **Step 2: Run the service tests to confirm the expected red state**

  Run `go test ./pkg/windowsync -run 'TestService'`. Expected result: failure because the service implementation is missing.

- [ ] **Step 3: Implement the minimum synchronized event loop**
...
- [ ] **Step 4: Run focused tests, then race-check the core**
...
- [ ] **Step 1: Add build-safe tests for the transition contract**
...
- [ ] **Step 2: Run platform tests before adding implementation**
...
- [ ] **Step 3: Implement `SendKey` by extracting the existing PostMessage lifecycle**
...
- [ ] **Step 4: Implement the Windows low-level hook adapter**
...
- [ ] **Step 5: Implement the non-Windows stubs and run compile tests**
...
- `app.FollowSyncService` exposes `GetSnapshot`, `SetRules`, `BeginMainWindowPick`, `BeginFollowWindowPick`, `RemoveFollowWindow`, `ClearTargets`, `Start`, and `Stop` as Wails methods.
- Wails events are `follow-sync:state`, `follow-sync:window-pick-preview`, `follow-sync:window-pick-complete`, and `follow-sync:error`; event payloads use `WindowInfo` and never include handles.

- [ ] **Step 1: Write the failing config and facade contract tests**
...
- [ ] **Step 3: Add the persisted rule field without persisting handles**
...
- [ ] **Step 4: Implement the facade and picker event routing**
...
- [ ] **Step 5: Bind the new service and regenerate/check bindings**
...
- The store subscribes to the four `follow-sync:*` events, disables rule/target mutations while `state === 'running'`, and exposes only `start`/`stop` for the whole sync session.
...
- [ ] **Step 1: Write failing utility and registry tests**
...
- [ ] **Step 2: Run the frontend tests to verify the new tab is absent**
...
- [ ] **Step 3: Implement the native API wrapper and store**

  Follow the existing `clicker-api.ts` availability pattern, map generated Wails models to local types, keep optimistic state updates only after successful native calls, and surface native errors with Ant Design `message`.

- [ ] **Step 4: Implement the bright Ant Design feature UI**

  Use existing theme tokens and icons. Make “添加跟随窗口” repeatable, show each follower as data only, render disabled inputs/buttons while running, and explain that handles must be rebound after restart. Do not add controls that can start/stop/cancel a follower independently.

- [ ] **Step 5: Register the feature and run the frontend checks**
...
- [ ] **Step 1: Document the runtime-only target model and capture boundary**
...
- [ ] **Step 2: Run the complete requested verification matrix**

  Run `go test ./...`, `go vet ./...`, `pnpm -r test`, `pnpm -r typecheck`, `pnpm -r build`, and `wails build -clean`. Record each exit code and any platform limitation explicitly.

- [ ] **Step 3: Inspect the final diff and protected-file status**
